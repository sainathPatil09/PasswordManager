...
const Manager = () => {
    ...
    const savePassword = async() => {
        if(form.site.length > 3 && form.password.length>3 && form.username.length>3){
            ...
            await fetch(`${apiUrl}/`, {method:"DELETE", headers:{"Content-Type":"application/json"},
    ...
    const deletePassword = async(id) =>{
        if(confirm("Are you sure want to delete password") ===true){
            ...
            let res = await fetch(`${apiUrl}/`, {method:"DELETE", headers:{"Content-Type":"application/json"},
        ...
        <>
            ...
            <div className=" mx-auto md:mycontainer">
                ...
                <div className="flex flex-col p-4 gap-4 items-center ">
                    <input value={form.site} onChange={handleChange} className=' w-full border border-green-400 p-1 rounded-full ' placeholder='Enter website URL' type="text" name='site' id='site' />
                    <div className="flex md:flex-row flex-col md:gap-12 gap-4 w-full relative ">

                        <input value={form.username} onChange={handleChange} className=' border border-green-400 p-1 rounded-full w-full' placeholder='Enter user name' type="text" name='username' id='username' />


                        <input ref={passwordRef} value={form.password} onChange={handleChange} className=' border border-green-400 p-1 rounded-full md:w-1/2 ' placeholder='Enter password' type="password" name='password' id='password' />
                        <span className='absolute right-3 top-1' onClick={handletoogleEye}>
                            <img ref={ref} className='text-black cursor-pointer' width={25} src="icons/eyecross.png" alt="eye" />
                        </span>
                    </div>
                    <button onClick={savePassword} className='flex justify-center items-center bg-green-500 rounded-full p-2 w-fit gap-2 font-bold hover:bg-green-400 '>
                        <lord-icon
